...
class create_graph:
    ...
    def create_single_graph(self, f1, in_train_set, in_test_set):
        ...
        carbon_list = ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9',
                       ...
                       'Neup_C1', 'Neup_C2', 'Neup_C3', 'Neup_C4', 'Neup_C5',
                       'Neup_C6', 'Neup_C7', 'Neup_C8', 'Neup_C9']
        ...
        embedding_matrix = torch.zeros([len(temp_df), len(self.atom_name_embed) + \
                                        ...
                                        len(self.monosaccharide_simple_embed)], dtype=torch.float32)
        # embedding_matrix = torch.zeros([len(temp_df), len(self.atom_embed) + len(self.mono_embed)], dtype=torch.float32)
        # embedding_matrix = torch.zeros([len(temp_df), len(self.atom_name_embed)], dtype=torch.float32)
        ...
        atom_name_list = temp_df['Merged_Atom_name'].values
        ...
        mono_simple_list = temp_df['Merged_standard_mono'].values
        ...
        for i in range(len(temp_df)):
            ...
            embedding_matrix[i, :] = torch.tensor(np.concatenate([c_atom_name_embed,
                                                                  ...
                                                                  c_bound_ab_embed,
                                                                  c_bound_dl_embed,
                                                                  c_carbon_pf_embed], axis=0))
            # embedding_matrix[i, :] = torch.tensor(np.concatenate([c_atom_name_embed], axis=0))
