            ...
            with torch.no_grad():
                for step, batch_data in enumerate(data_loader):
                    ...
                    preds = torch.cat([preds,
                                       out.detach_()[batch_data.train_carbon_mask]], dim=0)
                    targets = torch.cat([targets,
                                         batch_data.y.unsqueeze(1)[batch_data.train_carbon_mask]], dim=0)

                    # preds = torch.cat([preds,
                    #                    out.detach_()[batch_data.train_hydrogen_mask]], dim=0)
                    # targets = torch.cat([targets,
                    #                      batch_data.y.unsqueeze(1)[batch_data.train_hydrogen_mask]], dim=0)
